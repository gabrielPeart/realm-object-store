...
template<typename T, typename Context>
void List::insert(Context& ctx, size_t list_ndx, T value)
{
    dispatch([&](auto t) { insert(list_ndx, ctx.template unbox<std::decay_t<decltype(*t)>>(value, true)); });
}

template<typename T, typename Context>
void List::set(Context& ctx, size_t row_ndx, T value)
{
    dispatch([&](auto t) { set(row_ndx, ctx.template unbox<std::decay_t<decltype(*t)>>(value, true)); });
}
